...
public class Nativa_adec implements Instruction {
    ...
    public Object ejecutar(Enviorement env) {
        ...
        if (sym != null) {
            ...
            if (ob.compare(Objeto.Tipo.STRING)) {
                try {
                    return new Primitivo(Objeto.Tipo.DECIMAL, Double.parseDouble(ob.toString()));
                } catch (Exception ex) {
                ...
                try {
                    return new Primitivo(Objeto.Tipo.DECIMAL, Double.parseDouble(num));
                } catch (Exception ex) {
                    return new Primitivo(Objeto.Tipo.DECIMAL, 0.0);
                }
            } else {
